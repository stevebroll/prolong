...
# head(meltdf)
  ...
  ggplot2::ggplot(meltdf, ggplot2::aes(x = time, y = value, group = id)) + ggplot2::geom_line(color = colors) + ggplot2::theme(legend.position = "none") +ggplot2::xlab("Time") + ggplot2::ylab("Outcome")
...
```{r, cache = T}
promod_ll = prolong(sim_metabs, sim_outcome, groups = FALSE, lambda2 = 0.2374421, lambdar = 0.2229402)
```
...
```{r, cache = T}
promod_gll = prolong(sim_metabs, sim_outcome, lambda2 = 1.82, lambdar = 0.5, foldid = c(rep(rep(1:5,each = 3),3)))
```
