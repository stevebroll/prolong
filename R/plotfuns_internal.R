...
complexheatmap.2 <-
  function(x,
    ...
    if ("row" %in% scale) {
      ...
      message_wrap(
        "Note, in 'heatmap.2()', when rows are scaled, the row dendrogram is still calculated from the original matrix (not from the scaled matrix)."
      )
      ...
      message_wrap(
        "Note, in 'heatmap.2()', when columns are scaled, the column dendrogram is still calculated from the original matrix (not from the scaled matrix)."
      )
    }
    ht_param$matrix <- mat
    ...
    if (!missing(colsep)) {
      warning_wrap(
        "argument `colsep` is not supported in heatmap.2 -> Heatmap translation, skip it. Suggest to use `column_split` argument in Heatmap() which can be directly used here."
      )
    }
    if (!missing(rowsep)) {
      warning_wrap(
        "argument `rowsep` is not supported in heatmap.2 -> Heatmap translation, skip it. Suggest to use `row_split` argument in Heatmap() which can be directly used here."
      )
    }
    if (!missing(cellnote)) {
    ...
    if (!missing(ColSideColors)) {
      ht_param$top_annotation <- ComplexHeatmap::HeatmapAnnotation(
        column = ColSideColors,
        col = list(column = structure(
          unique(ColSideColors),
          names = unique(ColSideColors)
        )),
        show_legend = FALSE,
    ...
    if (!missing(RowSideColors)) {
      ...
      ComplexHeatmap::rowAnnotation(
        row = RowSideColors,
        col = list(row = structure(
          unique(RowSideColors),
          names = unique(RowSideColors)
        )),
        show_legend = FALSE,
        show_annotation_name = FALSE
      )
    ...
    ht_param$row_names_gp <-
      grid::gpar(fontsize = 12 * cexRow, col = colRow)
    if (is.null(colCol)) {
      colCol <- "black"
    }
      ...
      if (is.null(ht_param$column_labels)) {
        ...
        ht_param$bottom_annotation <- ComplexHeatmap::HeatmapAnnotation(
          colnames = ComplexHeatmap::anno_text(
            ht_param$column_labels,
            gp = ht_param$column_names_gp,
            rot = ht_param$column_names_rot
          ),
      ...
      if (is.null(ht_param$row_labels)) {
        ...
        ht_param$right_annotation <- ComplexHeatmap::rowAnnotation(
          rownames = ComplexHeatmap::anno_text(
            ht_param$row_labels,
            gp = ht_param$row_names_gp,
            rot = ht_param$row_names_rot
          ),
