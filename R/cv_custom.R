...
cv.gglasso_prolong <-
  function(x,
    ...
    gglasso.object <- gglasso::gglasso(x,
                                       y,
                                       group,
                                       lambda = lambda,
                                       delta = delta,
...
cvtype <- function(type.measure = "mse",
                   subclass = "elnet") {
  ...
  if (!match(type.measure, subclass.type, FALSE)) {
    ...
    warning(
      paste(
        "Only ",
        paste(subclass.type, collapse = ", "),
        " available as type.measure for ",
        model.name,
        " models; ",
        type.measure,
        " used instead",
...
cv.glmnet_prolong <-
  function(x,
    ...
    if (!is.null(lambda) && alignment == "fraction") {
      warning(
        "fraction of path alignment not available if lambda given as argument; switched to alignment=`lambda`"
      )
      alignment <- "lambda"
    }
    N <- nrow(x)
    ...
    which <- match(
      c("type.measure", "nfolds", "foldid", "grouped",
        "keep"),
      names(glmnet.call),
      FALSE
    )
