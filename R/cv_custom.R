...
cvtype <- function(type.measure = "mse", subclass = "elnet") {
  ...
  typenames <- c(
    deviance = devname, mse = "Mean-Squared Error",
...
cvcompute <- function (cvstuff, foldid, nlams)
{
  ...
  for (i in seq(nfolds)) {
    ...
    wi = weights[foldid == i]
    outmat[i, ] = apply(mati, 2, stats::weighted.mean, w = wi,
                        na.rm = TRUE)
...
cvstats <- function (cvstuff, foldid, nfolds, lambda, nz, grouped, ...)
{
  ...
  cvm = with(cvstuff, apply(cvraw, 2, stats::weighted.mean, w = weights,
                            na.rm = TRUE))
  cvsd = with(cvstuff, sqrt(apply(scale(cvraw, cvm, FALSE)^2,
                                  2, stats::weighted.mean, w = weights, na.rm = TRUE)/(N - 1)))
  nas = is.na(cvsd)
  if (any(nas)) {
    lambda = lambda[!nas]
    cvm = cvm[!nas]
...
cv.glmnet.raw <- function(x, y, weights, offset, lambda, type.measure, nfolds,
                          foldid, alignment, grouped, keep, parallel, trace.it, glmnet.call,
                          cv.call, ...) {
  if (trace.it) {
    cat("Training\n")
  }
  glmnet.object <- glmnet::glmnet(x, y,
    weights = weights, offset = offset,
    lambda = lambda, trace.it = trace.it, intercept = F, ...
  )
  ...
  if (inherits(glmnet.object, "multnet") && !glmnet.object$grouped) {
    ...
    nz <- sapply(
      stats::predict(glmnet.object, type = "nonzero"),
      length
    )
  }
  outlist <- as.list(seq(nfolds))
  ...
  if (parallel) {
    `%dopar%` <- foreach::`%dopar%`
    outlist <- foreach::foreach(i = seq(nfolds),
      ...
      glmnet::glmnet(x[-fold, , drop = FALSE], y_sub,
        lambda = lambda,
    ...
    for (i in seq(nfolds)) {
      ...
      outlist[[i]] <- glmnet::glmnet(x[-fold, , drop = FALSE],
        y_sub,
        lambda = lambda, offset = offset_sub,
        weights = weights[-fold], trace.it = trace.it, intercept = F,
        ...
      )
    }
  }
  ...
  predmat <- glmnet::buildPredmat(outlist, lambda, x, offset, foldid,
    alignment,
    y = y, weights = weights, grouped = grouped,
    type.measure = type.measure, family = stats::family(glmnet.object)
  )
  fun <- paste("cv", subclass, sep = ".")
  cvstuff <- do.call(fun, list(
    predmat, y, type.measure, weights,
    foldid, grouped
  ))
...
cv.glmnet_prolong <- function(x, y, weights = NULL, offset = NULL, lambda = NULL,
                              type.measure = c(
                                ...
                                "auc", "mae", "C"
                              ), nfolds = NULL, foldid = NULL, alignment = c(
                                "lambda",
                                "fraction"
  ...
  cv.glmnet.raw(
    x, y, weights, offset, lambda, type.measure,
